    ...
    var executor = function(args, success, failure) {
      ...
      var gap = 80;
        ...
        if ((xCount > 1) && (yCount > 1) && (scale > 0)){
            ...
            var svgheader ='<?xml version="1.0" encoding="utf-8"?><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="'+width+'px" height="'+height+'px" viewBox="0 0 '+((xCount*200)+25)+' '+((yCount*200)+28)+'" xml:space="preserve">';
            var svgend = '</svg>';
            ...
            if(((xCount%2)==0)&&((yCount%2)==0)){
                svgOut=svgOut.concat(getPathData(1,(xCount-1)*200,(yCount-1)*200,2));
                svgOut=svgOut.concat(getPathData(1,(xCount-1)*200,0,3));
                svgOut=svgOut.concat(getPathData(1,0,(yCount-1)*200,1));
            }else if((xCount%2)==0){
                svgOut=svgOut.concat(getPathData(1,(xCount-1)*200,0,3));
                svgOut=svgOut.concat(getPathData(5,(xCount-1)*200,(yCount-1)*200,2));
                svgOut=svgOut.concat(getPathData(5,0,(yCount-1)*200,1));
            }else if((yCount%2)==0){
                svgOut=svgOut.concat(getPathData(1,0,(yCount-1)*200,1));
                svgOut=svgOut.concat(getPathData(5,(xCount-1)*200,0,3));
                svgOut=svgOut.concat(getPathData(5,(xCount-1)*200,(yCount-1)*200,2));
            }else{
                svgOut=svgOut.concat(getPathData(1,(xCount-1)*200,(yCount-1)*200,2));
                svgOut=svgOut.concat(getPathData(5,0,(yCount-1)*200,1));
            ...
            //bottom side
            for (x=2;x<xCount;x++){
                svgOut=svgOut.concat(getPathData(2+(x%2),(x-1)*200,0,0));
            }
            //left side
            for (y=2;y<yCount;y++){
                svgOut=svgOut.concat(getPathData(3-(y%2),0,(y-1)*200,1));
            }
            //right side
            for (x=2;x<xCount;x++){
                svgOut=svgOut.concat(getPathData(3-((x+(yCount%2))%2),(x-1)*200,(yCount-1)*200,2));
            }
            //top side
            for (y=2;y<yCount;y++){
                svgOut=svgOut.concat(getPathData(2+((y+(xCount%2))%2),(xCount-1)*200,(y-1)*200,3));
            }
            //fill middle
            for (x=2; x<xCount;x++){
                var k=0;
                if((x%2)==0) k=1;
                for(y=2;y<yCount;y++){
                    svgOut=svgOut.concat(getPathData(4,(x-1)*200,(y-1)*200,(x+(y%2))%2));
                }
            }
      }
      success(svgOut);
